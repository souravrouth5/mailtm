...
class MailTMClient
{
    ...
    public function deleteMessage(string $messageId): bool
    {
        ...
        return $this->requestWithStatus('DELETE', "/messages/{$messageId}", [], $this->token, [200, 204]);
    }

    public function markAsRead(string $messageId): bool
    {
        if (!$this->token) return false;
        $response = $this->request('PATCH', "/messages/{$messageId}", ['seen' => true], $this->token);
    ...
    private function request(string $method, string $endpoint, array $data = [], ?string $token = null): ?array
    {
        ...
        if (in_array($method, ['POST', 'PATCH']) && $data) {
    ...
    private function requestWithStatus(string $method, string $endpoint, array $data = [], ?string $token = null, array $successCodes = [200]): bool
    {
        $url = $this->baseUrl . $endpoint;
        $ch = curl_init($url);
        ...
        return in_array($httpCode, $successCodes);
    }
}
...
class OTPMonitor
{
    ...
    public function __construct(MailTMClient $client, int $checkInterval = 5, callable $outputCallback = null)
    {
        ...
        $this->outputCallback = $outputCallback ?? function ($msg) {
            echo $msg . PHP_EOL;
        };
    }
